...
const SchoolLogin = () => {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const userCredential = await signInWithEmailAndPassword(auth, formData.coordinatorEmail, formData.password);
      navigate('/dashboard', { state: { authUid: userCredential.user.uid } });
    } catch (error) {
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') {
        setLoginMessage('The coordinator email or password is incorrect.');
      } else {
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center relative py-24 px-6 lg:px-16">
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            <h1 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">School Login</h1>
            <p className="text-slate-600 font-medium text-base mt-3">Enter your credentials to continue</p>
          </div>

          <form className="flex flex-col gap-6" onSubmit={handleLogin}>
            <div className="flex flex-col gap-2">
              <label className="text-sm font-bold text-slate-800 ml-1">Coordinator Email</label>
              <input 
            ...
            <div className="flex justify-end items-center px-1">
              <a href="#" className="text-sm font-bold text-blue-700 hover:text-blue-800 transition-colors">Forgot Password?</a>
            </div>

            {loginMessage && <p role="alert" className="text-sm font-semibold text-red-700">{loginMessage}</p>}

            <button type="submit" disabled={isSubmitting} className="w-full py-4 rounded-full bg-blue-700 text-white font-bold hover:bg-blue-800 transition-all shadow-md hover:shadow-lg mt-4 disabled:cursor-not-allowed disabled:opacity-60">
              {isSubmitting ? 'Logging in...' : 'Login to Dashboard'}
            </button>
          </form>

          <p className="text-center mt-12 text-sm text-slate-600 font-medium">
            Not registered yet? <Link to="/register" className="text-blue-700 font-bold hover:underline">Register School</Link>
          </p>
        </div>
