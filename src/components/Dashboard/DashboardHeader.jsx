...
const DashboardHeader = ({ schoolInfo }) => {
  return (
    <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4 flex flex-wrap items-center justify-between gap-4 z-50 shadow-sm">
      ...
      {/* Right: School identity */}
      <div className="text-right">
        <h1 className="text-xl font-extrabold text-slate-900 leading-tight">{schoolInfo.name}</h1>
        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-0.5">UDISE: {schoolInfo.udise}</p>
      </div>
    </header>
  );
};
